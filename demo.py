...
#     fw.write('Rank_Index=')
#     fw.write(str(b))
#     fw.close()  # 由于我这里使用二维数组进行比对，所以为了方便的获取二维数组中的部分内容，定义了一个取这些内容的函数，参数 i,j是行的起始和终止列，参数k，m是列的起始和终止列
...
#             a[(Get_Realnum(Result_Map[i][0], Rank_Index))] = round(
#                     (a[(Get_Realnum(Result_Map[i][0], Rank_Index))] + 0.2), 2)
#         elif Result_Map[i][1] <= 0.05:  # 如果矩阵距离很小，说明相似度很高，要人为加大其概率
#             a[(Get_Realnum(Result_Map[i][0], Rank_Index))] = round(
#                     (a[(Get_Realnum(Result_Map[i][0], Rank_Index))] + 0.4), 2)
#     return a
...
#     elif X_Std > 2.0 and Y_Std > 2.0 and X_MaxIndex > round(float(len(Image_Value[0])) / 2.0, 2) and Y_MaxIndex < round(
#                     float(len(Image_Value)) / 2.0, 2):
